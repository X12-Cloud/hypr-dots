...
hl.bind("SUPER + F", function()
	hl.dispatch(hl.dsp.window.resize({ x = 1000, y = 900 }))
end)
hl.bind("SUPER + L", hl.dsp.exec_cmd("wlogout -b 3"))
hl.bind(
	"SUPER + SHIFT + S",
	hl.dsp.exec_cmd(
		"grim -g \"$(slurp)\" - | tee ~/Pictures/Screenshots/$(date +'%Y%m%d_%Hh%Mms%s_grim.png') | wl-copy"
	)
)
hl.bind(
	"SHIFT + Print",
...
hl.bind("Print", hl.dsp.exec_cmd("grim - | tee ~/Screenshots/$(date +'%Y%m%d_%Hh%Mms%s_grim.png') | wl-copy"))
